...
func main() {
	/// Подлключение системы логирования \\\
	logger := logger.GetLogger()
	logger.Info("logger initialized")

	/// Подлключение конфигурации приложения \\\
	cfg := config.GetConfig()
	logger.Info("loaded config file")

	/// Подлключение маршрутизатора для обработки HTTP запросов \\\
	router := httprouter.New()
	logger.Info("initialized httprouter")

	/// Подлключение к БД Postgresql \\\
	...
	/// Работа с сигналами и каналами приложения \\\
	...
	logger.Infof("server has been started on host %s and port %s", cfg.HTTP.Host, cfg.HTTP.Port)

	/// Получение значений из канала \\\
	<-quit
	logger.Warn("shutting down the server")

	/// Закрытие БД и освобождение связанных ресурсов \\\
...
///Тестовое визуальное представление работы приложения \\\
